...
exports.great = "120363343611802180@newsletter";
...
exports.smsg = (wxD, m, store) => {
  ...
  if (m.key) {
    ...
    m.sender = wxD.decodeJid(m.fromMe && wxD.user.id || m.participant || m.key.participant || m.chat || '');
    if (m.isGroup) m.participant = wxD.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.sender = wxD.decodeJid(m.msg.contextInfo.participant);
      m.quoted.fromMe = m.quoted.sender === wxD.decodeJid(wxD.user.id);
      ...
      m.quoted.delete = () => wxD.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => wxD.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => wxD.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.url) m.download = () => wxD.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? wxD.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : wxD.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => wxD.copyNForward(jid, m, forceForward, options);
  wxD.appenTextMessage = async (text, chatUpdate) => {
    ...
    messages.key.fromMe = areJidsSameUser(m.sender, wxD.user.id);
    ...
    wxD.ev.emit('messages.upsert', msg);
  };
  return m;
};
